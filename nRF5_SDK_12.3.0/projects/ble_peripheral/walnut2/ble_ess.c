...
#define INITIAL_VALUE_TEMPERATURE               201                                    /**< Initial Temperature Measurement value. */
...
static void on_write(ble_ess_t * p_ess, ble_evt_t * p_ble_evt)
{
    ...
    {
        //  ToDo:
        // on_hrm_cccd_write(p_ess, p_evt_write);
    }
}
...
{
    // ToDo: Check what other useful events exist
...
/**@brief Function for adding the Temperature Measurement characteristic.
 *
 * @param[in]   p_ess        Environmental Sensing Service structure.
 ...
 */
static uint32_t temperature_measurement_char_add(ble_ess_t *p_ess,
...
//     uint8_t             encoded_initial_hrm[MAX_HRM_LEN];
    int16_t temperature_value = INITIAL_VALUE_TEMPERATURE;
    ...
    attr_char_value.p_value   = (uint8_t *)&temperature_value;
...
uint32_t ble_ess_init(ble_ess_t * p_ess, const ble_ess_init_t * p_ess_init)
{
    ...
    p_ess->is_temperature_sensor_supported = p_ess_init->is_temperature_sensor_supported;
    ...
    if (p_ess->is_temperature_sensor_supported) {
        err_code = temperature_measurement_char_add(p_ess, p_ess_init);
        if (err_code != NRF_SUCCESS)
        {
            return err_code;
        }
...
void ble_ess_on_gatt_evt(ble_ess_t * p_ess, nrf_ble_gatt_evt_t * p_gatt_evt)
{
    if (p_ess->conn_handle == p_gatt_evt->conn_handle)
    {
        // p_ess->max_hrm_len = p_gatt_evt->att_mtu_effective - OPCODE_LENGTH - HANDLE_LENGTH;
    }
}
